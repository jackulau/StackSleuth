import { FastAPIAgent, FastAPIRouteMetrics, FastAPIPerformanceStats } from '../index';

describe('FastAPIAgent', () => {
  let agent: FastAPIAgent;

  beforeEach(() => {
    // Create agent with autoInit: false to prevent hanging
    agent = new FastAPIAgent({
  ...
  describe('constructor', () => {
    it('should create a new FastAPIAgent instance', () => {
      expect(agent).toBeInstanceOf(FastAPIAgent);
    });

    it('should accept configuration options', () => {
      const configuredAgent = new FastAPIAgent({
        slowQueryThreshold: 500,
        maxMetricsHistory: 500,
        pythonServerUrl: 'http://localhost:9000',
        autoInit: false
      });
      expect(configuredAgent).toBeInstanceOf(FastAPIAgent);
    });
  });
