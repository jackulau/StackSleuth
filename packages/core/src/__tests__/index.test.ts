...
describe('ProfilerCore', () => {
  ...
  beforeEach(() => {
    profiler = new ProfilerCore({ output: { console: false } });
  });
  ...
  describe('initialization', () => {
    ...
    it('should accept custom configuration', async () => {
      const customProfiler = new ProfilerCore({
        enabled: true,
        sampling: { rate: 0.5 },
  ...
  describe('trace management', () => {
    ...
    it('should start and complete a trace', () => {
      ...
      profiler.completeTrace(traceId!, TraceStatus.SUCCESS);
      ...
      return new Promise<void>((resolve) => {
        setTimeout(() => {
          ...
          expect(trace?.status).toBe(TraceStatus.SUCCESS);
          resolve();
        }, 10);
      });
    });
    ...
    it('should complete trace with error status', () => {
      ...
      profiler.completeTrace(traceId!, TraceStatus.ERROR);

      return new Promise<void>((resolve) => {
        setTimeout(() => {
          const traces = profiler.getTraces();
          const trace = traces.find(t => t.id === traceId);
          expect(trace?.status).toBe(TraceStatus.ERROR);
          resolve();
        }, 10);
      });
    });
  ...
  describe('span management', () => {
    ...
    it('should start and complete a span', () => {
      const traceId = profiler.startTrace('test-trace');
      const spanId = profiler.startSpan(traceId!, 'test-span', SpanType.FUNCTION_CALL);

      expect(spanId).toBeTruthy();

      profiler.completeSpan(spanId!, TraceStatus.SUCCESS);
    ...
    it('should handle nested spans', () => {
      ...
      const parentSpanId = profiler.startSpan(traceId!, 'parent-span', SpanType.HTTP_REQUEST);
      const childSpanId = profiler.startSpan(traceId!, 'child-span', SpanType.DB_QUERY, parentSpanId!);

      expect(childSpanId).toBeTruthy();

      profiler.completeSpan(childSpanId!, TraceStatus.SUCCESS);
      profiler.completeSpan(parentSpanId!, TraceStatus.SUCCESS);
      profiler.completeTrace(traceId!);
    ...
    it('should record an error', () => {
      ...
      return new Promise<void>((resolve) => {
        setTimeout(() => {
          ...
          expect(errorTrace?.status).toBe(TraceStatus.ERROR);
          resolve();
        }, 10);
      });
    });
...
describe('TraceCollector', () => {
  ...
  beforeEach(() => {
    collector = new TraceCollector({ enabled: true, sampling: { rate: 1.0 } });
  });
  ...
  describe('trace lifecycle', () => {
    ...
    it('should complete a trace', () => {
      ...
      expect(completedTrace?.status).toBe(TraceStatus.SUCCESS);
    ...
    it('should respect sampling rate', () => {
      const samplingCollector = new TraceCollector({
        enabled: true,
  ...
  describe('span lifecycle', () => {
    it('should start a span within a trace', () => {
      const trace = collector.startTrace('test-trace');
      const span = collector.startSpan(trace!.id, 'test-span', SpanType.FUNCTION_CALL);
    ...
    it('should complete a span', () => {
      const trace = collector.startTrace('test-trace');
      const span = collector.startSpan(trace!.id, 'test-span', SpanType.DB_QUERY);

      collector.completeSpan(span!.id, TraceStatus.SUCCESS, { extra: 'data' });

      const completedTrace = collector.getTrace(trace!.id);
      const completedSpan = completedTrace?.spans.find(s => s.id === span?.id);

      expect(completedSpan?.status).toBe(TraceStatus.SUCCESS);
      expect(completedSpan?.timing.duration).toBeDefined();
    });

    it('should add error to span', () => {
      const trace = collector.startTrace('test-trace');
      const span = collector.startSpan(trace!.id, 'error-span', SpanType.HTTP_REQUEST);
      ...
      expect(errorSpan?.status).toBe(TraceStatus.ERROR);
    });
  });
  ...
  describe('events', () => {
    ...
    it('should emit span:started event', (done) => {
      ...
      collector.startSpan(trace!.id, 'event-span', SpanType.CUSTOM);
    });
  });
});
...
describe('Timer', () => {
  ...
  it('should calculate diff between timestamps', () => {
    ...
    const diff = Timer.diff(start, end);
...
describe('IdGenerator', () => {
  it('should generate unique trace IDs', () => {
    const id1 = IdGenerator.traceId();
    const id2 = IdGenerator.traceId();
  ...
  it('should generate unique span IDs', () => {
    const id1 = IdGenerator.spanId();
    const id2 = IdGenerator.spanId();
  ...
  it('should generate short IDs', () => {
    const fullId = IdGenerator.traceId();
    const shortId = IdGenerator.shortId(fullId);
...
describe('PerformanceUtils', () => {
  describe('calculatePercentile', () => {
    it('should calculate percentile correctly', () => {
      ...
      expect(PerformanceUtils.calculatePercentile(values, 50)).toBe(5.5);
      expect(PerformanceUtils.calculatePercentile(values, 0)).toBe(1);
      expect(PerformanceUtils.calculatePercentile(values, 100)).toBe(10);
    });
  ...
  describe('calculateStats', () => {
    it('should calculate all stats', () => {
      ...
      const stats = PerformanceUtils.calculateStats(values);
    ...
    it('should return zeros for empty array', () => {
      const stats = PerformanceUtils.calculateStats([]);
  ...
  describe('formatDuration', () => {
    it('should format microseconds', () => {
      expect(PerformanceUtils.formatDuration(0.5)).toContain('μs');
    });

    it('should format milliseconds', () => {
      expect(PerformanceUtils.formatDuration(500)).toContain('ms');
    });

    it('should format seconds', () => {
      expect(PerformanceUtils.formatDuration(2000)).toContain('s');
    });
  });

  describe('formatBytes', () => {
    it('should format bytes', () => {
      expect(PerformanceUtils.formatBytes(500)).toContain('B');
    });

    it('should format kilobytes', () => {
      expect(PerformanceUtils.formatBytes(1500)).toContain('KB');
    });

    it('should format megabytes', () => {
      expect(PerformanceUtils.formatBytes(1500000)).toContain('MB');
    });
  });
});
...
describe('SamplingUtils', () => {
  describe('shouldSample', () => {
    it('should always sample at rate 1.0', () => {
      let sampled = 0;
      for (let i = 0; i < 100; i++) {
        if (SamplingUtils.shouldSample(1.0)) sampled++;
      }
    ...
    it('should never sample at rate 0', () => {
      let sampled = 0;
      for (let i = 0; i < 100; i++) {
        if (SamplingUtils.shouldSample(0)) sampled++;
      }
  ...
  describe('createThrottler', () => {
    it('should throttle requests', () => {
      const throttler = SamplingUtils.createThrottler(5);
    ...
    it('should refill tokens after one second', (done) => {
      const throttler = SamplingUtils.createThrottler(2);
